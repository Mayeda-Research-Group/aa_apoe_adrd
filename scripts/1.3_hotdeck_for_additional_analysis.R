...
#---- check age dist ----
aa_apoe_tte_selected %>% 
  ggplot(aes(x = survey_age)) + 
  geom_density()

aa_apoe_tte_selected %>% reframe(median(survey_age)) 

aa_apoe_tte_selected <- aa_apoe_tte_selected %>% 
  mutate(age_le69 = ifelse(survey_age <= 69, 1, 0))
...
aa_apoe_long_tte_selected <- aa_apoe_long_tte_selected %>% 
  ...
  left_join(impute_data %>% select(subjid, edu_3, usaborn_rev, income_pp), by = "subjid") 
