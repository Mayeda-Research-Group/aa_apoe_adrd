...
for (i in 1:length(race_varlabel)){
  ...
  temp2 <- temp %>% 
    ...
    mutate(fu_yr = as.numeric(fu_yr)) %>%
    arrange(fu_yr, estimate)
  RD_RR_time_CI <- bind_rows(RD_RR_time_CI, temp2, temp1)
}
...
# # Check Filipino
# RD_RR_time_CI %>%
#   filter(race == "Filipino", fu_yr == 10) %>%
#   select(estimate, race, matches("mean_cif\\d_model_1"), RR_model_1, RD_model_1, 
#          excsfrac_model_1) %>% view()
# Check all groups
# RD_RR_time_CI %>%
#   filter(fu_yr == 10) %>%
#   select(estimate, race, matches("mean_cif\\d_model_1"), RR_model_1, RD_model_1,
...
RD_RR_time_CI_model_long %>% 
  ...
  scale_linetype_manual(
    ...
    labels = c(expression(paste(italic("APOE-"), italic(epsilon), italic("4"), " carriers")),
               "Non-carriers")) +
  scale_y_continuous(limits = c(0, 73), breaks = seq(0, 70, 10)) +
